...
public final class AlarmInstance implements ClockContract.InstancesColumns {
    ...
    public static Intent createIntent(Context context, Class<?> cls, long instanceId) {
		Log.i("ClockShutdown,api:new Intent(context, Class<?> cls).setData(getUri(long instanceId)");//liuqipeng add
   ...
   /**
     * Get an alarm instances by alarm time.
     *
     * @param contentResolver to perform the query on.
     * @param alarmId of instances desired.
     * @return list of alarms instances that are owned by alarmId.
     */
    public static List<AlarmInstance> getInstancesByAlarmTime(ContentResolver contentResolver,
            int hour, int minutes) {
        return getInstances(contentResolver, HOUR + "=" + hour + " and " + MINUTES + "=" + minutes);
    }
    ...
    public static boolean updateInstance(ContentResolver contentResolver, AlarmInstance instance) {
        ...
        Log.i("alarm time is : " + instance.getAlarmTime());
        ContentValues values = createContentValues(instance);
...
//liuqipeng begin
			...
			mRingtone = RingtoneManager.getDefaultUri(RingtoneManager.TYPE_ALARM);
			//mRingtone =	Uri.parse("content://media/internal/audio/media/15");//实际这行if并没执行到，所以只有写在下面才生效
//liuqipeng
        } else {
			//mRingtone =Uri.parse("content://media/internal/audio/media/15");
