...
public class AliAlarmScreen extends FrameLayout {
    ...
    private static final String CONFIG_FILE_NAME = "alarmstyle";
    ...
    public AliAlarmScreen(Context context) {
        super(context);
        init();
    }

    public AliAlarmScreen(Context context, AttributeSet attrs) {
        super(context, attrs);
        init();
    }

    public AliAlarmScreen(Context context, AttributeSet attrs, int defStyle) {
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                ...
                case MSG_SNOOZE:
                    ...
                    TA.getInstance().getDefaultTracker().commitEvent("Page_AlarmAlerting",2101, "Page_AlarmAlert_Button_Long_Press_to_Set_Snooze_Duration", null, String.valueOf(interval), null);
                    //mAlarmView.getViewRootImpl().cancelInvalidate(mAlarmView);
    ...
    private static class AlarmElementRoot extends FancyElementRoot implements OnExternCommandListener {
        ...
        public void onCommand(String command, Double para1, String para2) {
            if ("snooze_alarm".equals(command)) {
                Message msg = getContext().getHandler().obtainMessage(MSG_SNOOZE);
                if (para1 != null)
                    msg.arg1 = para1.intValue();
                msg.sendToTarget();
            } else if (AlarmFeatureOption.YUNOS_QCOM_PLATFORM && "power_off".equals(command)) {
                getContext().getHandler().obtainMessage(MSG_POWER_OFF).sendToTarget();
            } else {
                getContext().getHandler().obtainMessage(MSG_DISMISS).sendToTarget();
            }
    ...
    public void onResume() {
        ...
        aliyun.aml.util.Utils.putVariableString("snooze_message", mRoot.getVariables(),
                getResources().getString(R.string.alarm_alert_snooze_text));
        String label = ((AlarmActivity) getContext()).getAlarmInstance().mLabel;
        Utils.putVariableString("alarm_label", mRoot.getVariables(), label);
    }
}
