...
public class TimerObj implements Parcelable {
    ...
    private static final String PREF_TIMER_ISTAG = "timer_istag_";
    ...
    public void writeToSharedPref(SharedPreferences prefs) {
        ...
        key = PREF_TIMER_ISTAG + id;
        editor.putBoolean(key, isTag);
        editor.apply();
    }
    ...
    public void readFromSharedPref(SharedPreferences prefs) {
        ...
        key = PREF_TIMER_ISTAG + id;
        isTag = prefs.getBoolean(key, false);
    }
    ...
    public static TimerObj getTagTimersFromSharedPrefs(
            SharedPreferences prefs, boolean match) {
        Object[] timerStrings = prefs.getStringSet(PREF_TIMERS_LIST, new HashSet<String>())
                .toArray();
        if (timerStrings.length > 0) {
            for (int i = 0; i < timerStrings.length; i++) {
    ...
    public static TimerObj removeTimeUpTagTimersFromSharedPrefs(
            SharedPreferences prefs, boolean match) {
        Object[] timerStrings = prefs.getStringSet(PREF_TIMERS_LIST, new HashSet<String>())
                .toArray();
        if (timerStrings.length > 0) {
            for (int i = 0; i < timerStrings.length; i++) {
                ...
                t.mTimerId = Integer.parseInt((String) timerStrings[i]);
                t.readFromSharedPref(prefs);
                if (t.isTag == true && t.mState == TimerObj.STATE_TIMESUP) {
                    return t;
                }
            }
        }
    ...
    public static void resetTimersInSharedPrefs(SharedPreferences prefs) {
        ...
        if (prefs.getBoolean("isRunning",false)) {
            Log.d(TAG, "mPrefs.getBoolean = "+prefs.getBoolean("isRunning",false));
