...
import com.android.deskclock.holiday.HolidayManager;
...
public final class AlarmStateManager extends BroadcastReceiver {
    ...
    ///@}
    /*YunOS BEGIN PB*/
    //##module:DeskClock##author:hanneng.yehn@alibaba-inc.com
    //##BugID:(6639047) ##date:2015-11-30
    //##description:support QCOM PowerOff alarm
    public static final int RTC_POWEROFF_WAKEUP = 5;//AlarmManager.RTC_POWEROFF_WAKEUP
    /*YunOS END PB*/
    /* YunOS BEGIN PB */
    //##module:DeskClock##author:xingwei.lxw@alibaba-inc.com
    //##BugID:(177312) ##date:2014-12-28
    //##description:Raise the priority of the receiver AlarmInitReceiver to promptly took over intents
    // Buffer time in seconds to fire alarm instead of marking it missed.
    public static final int ALARM_FIRE_BUFFER = AlarmFeatureOption.YUNOS_QCOM_PLATFORM ? 119 : 59;
    /*YunOS END PB*/
/*    public static int getGlobalIntentId(Context context) {
    ...
    public static void updateNextAlarm(Context context) {
        ...
        /// M: Enable power-off alarm when device Unencrypted. @{
        if (AlarmFeatureOption.YUNOS_MTK_PLATFORM && nextAlarm != null && PowerOffAlarm.deviceUnencrypted()) {
    ...
    public static Intent createStateChangeIntent(Context context, String tag,
		...
		Log.i("ClockShutdown,api:createIntent(Context context, Class<?> cls, long instanceId)");//liuqipeng add
        ...
        /*YunOS BEGIN PB*/
        //##module:DeskClock##author:xingwei.lxw@alibaba-inc.com
        //##BugID:(180776) ##date:2015-01-05
        //##description:run at foreground priority with a shorter timeout interval to receive alarm intent timely
    ...
    private static void scheduleInstanceStateChange(Context context, Calendar time,
        ...
        /** M: If the alarm will be fired next time, set the type as powerOffAlarm,
        but if it's tablet multi-user, set as normal alarm */
        //penglei hack this for the snooze time error.GOD HELP ME.
//        if (newState == AlarmInstance.FIRED_STATE) {
//              Log.v("Alarm has been set by powerOffAlarm");
//              return;
//        }

        Log.i("check the state got in here is : " + newState);
        // TODO penglei: this was used for timeout.
        ...
        //##description:support QCOM PowerOff alarm
			...
			Log.i("ClockShutdown,getstateChangePowerOffIntent");//liuqipeng add
            ...
            if (Utils.isKitKatOrLater()) {
				...
				Log.i("ClockShutdown,AlarmManager.RTC_POWEROFF_WAKEUP mode,exactExcute ");//liuqipeng add
                am.setExact(RTC_POWEROFF_WAKEUP, timeInMillis, pendingPowerOffIntent);
            } else {
                am.set(AlarmManager.RTC_WAKEUP, timeInMillis, pendingIntent);
				Log.i("ClockShutdown,AlarmManager.RTC_POWEROFF_WAKEUP mode,notexactExcute " );//liuqipeng add
                am.set(RTC_POWEROFF_WAKEUP, timeInMillis, pendingPowerOffIntent);			
            }
        /*YunOS END PB*/
        } else {
            am.set(AlarmManager.RTC_WAKEUP, timeInMillis, pendingIntent);
    ...
    private static void cancelScheduledInstance(Context context, AlarmInstance instance) {
        ...
        if (AlarmFeatureOption.YUNOS_QCOM_PLATFORM) {
            PendingIntent pendingPowerOffIntent = PendingIntent.getBroadcast(context,
                    instance.hashCode(), createStateChangeIntent(context, ALARM_MANAGER_TAG,instance, -1),
        ...
        if (AlarmFeatureOption.YUNOS_MTK_PLATFORM) {
            ...
            ClassUtils.invokeMethod(am, "cancelPoweroffAlarm", String.class, context.getPackageName());
            ///@}
        }
    }
    ...
    public static void setFiredState(Context context, AlarmInstance instance) {
        ...
        if (timeout != null) {
            ...
            //    Log.i("check the timeout is " + timeout);
            //} else {
...
//        Calendar powerOffCurrentTime = Calendar.getInstance();
//        Calendar powerOffTimeoutTime = instance.getTimeout(context);
//        boolean hasTimeout = powerOffTimeoutTime != null && powerOffCurrentTime.after(powerOffTimeoutTime);
//        Log.v("PowerOffAlarm setMissedState hasTimeout = " + hasTimeout);
        ...
        //String snoozeMinutesStr = snoozeTime;
        String snoozeMinutesStr = PreferenceManager.getDefaultSharedPreferences(context)
                .getString("SnoozeTime", DEFAULT_SNOOZE_MINUTES);
        PreferenceManager.getDefaultSharedPreferences(context).edit()
                .remove("SnoozeTime").commit();
        int snoozeMinutes = Integer.parseInt(snoozeMinutesStr);
        Calendar newAlarmTime = Calendar.getInstance();
        if (AlarmService.sSnoozeInterval != 0) {
            snoozeMinutes = AlarmService.sSnoozeInterval;
            AlarmService.sSnoozeInterval = 0;
        }
        Log.i("snooze time is : "+ snoozeMinutes);
...
//        }
    ...
    public static void setMissedState(Context context, AlarmInstance instance) {
        ...
        if (AlarmFeatureOption.YUNOS_MTK_PLATFORM) {
            if (PowerOffAlarm.bootFromPoweroffAlarm()) {
                ...
                Log.v("PowerOffAlarm setMissedState hasTimeout = " + hasTimeout);
                if (hasTimeout) {
                    Log.v("PowerOffAlarm timeout and set the alarm to missed state");
                    Intent powerOffAlarmIntent = new Intent();
                    powerOffAlarmIntent.setAction(AlarmService.POWER_OFF_ALARM_MISSED_ACITION);
                    context.sendBroadcast(powerOffAlarmIntent);
                    return;
                }
            }
     ...
     * @param instance to set state to
     */
    public static void setMissedStateForPoweroffAlarm(Context context, AlarmInstance instance) {
        Log.v("setMissedStateForPoweroffAlarm Setting missed state to instance " + instance.mId);
        // Stop alarm if this instance is firing it
        AlarmService.stopAlarm(context, instance);
    ...
    public static void registerInstance(Context context, AlarmInstance instance,
        ...
        if (currentTime.after(missedTTL)) {
            ...
            if (currentTime.before(alarmBuffer)) {
                ...
                Utils.sHideNotification = false;
                Log.v("Utils.sHideNotification = "+Utils.sHideNotification);
    ...
    /**
     * This will delete and unregister all instances associated with alarm time, without affect
     * the alarm itself. This should be used whenever modifying or deleting an alarm.
     *
     * @param context application context
     * @param alarmId to find instances to delete.
    ...
    public static void deleteAllInstancesByTime(Context context, int hour, int minutes) {
        ContentResolver cr = context.getContentResolver();
        List<AlarmInstance> instances = AlarmInstance.getInstancesByAlarmTime(cr, hour, minutes);
        for (AlarmInstance instance : instances) {
            unregisterInstance(context, instance);
            AlarmInstance.deleteInstance(context.getContentResolver(), instance.mId);
        }
    ...
    public static void fixAlarmInstances(Context context) {
        ...
        for (AlarmInstance instance : AlarmInstance.getInstances(contentResolver, null)) {
            Log.d("Utils.sChangByTime = " + Utils.sChangByTime + ", instance.mAlarmState = "+instance.mAlarmState);
            if (Utils.sChangByTime && instance.mAlarmState < AlarmInstance.FIRED_STATE) {
                Calendar currentTime = Calendar.getInstance();
                Calendar alarmTime = instance.getAlarmTime();
                Log.d("currentTime = "+currentTime+", alarmTime = "+alarmTime);
                if (alarmTime.after(currentTime)) {
                    Alarm alarm = Alarm.getAlarm(contentResolver, instance.mAlarmId);
                    int nYear = currentTime.get(Calendar.YEAR);
                    int nDay = currentTime.get(Calendar.DAY_OF_YEAR);
                    currentTime.set(Calendar.YEAR, alarmTime.get(Calendar.YEAR));
                    currentTime.set(Calendar.DAY_OF_YEAR, alarmTime.get(Calendar.DAY_OF_YEAR));
                    if (alarmTime.after(currentTime)) {
                        alarmTime.set(Calendar.YEAR, nYear);
                        alarmTime.set(Calendar.DAY_OF_YEAR, nDay);
                    } else {
                        nDay += 1;
                        alarmTime.set(Calendar.YEAR, nYear);
                        alarmTime.set(Calendar.DAY_OF_YEAR, nDay);
                    }
        if (alarm.daysOfWeek.mBitSet == alarm.daysOfWeek.LEGAL_DAYS) {
            HolidayManager holidayMgr = HolidayManager.getInstance(context);
            boolean isWeekend = false;
            boolean isHoliday = holidayMgr.isHoliday(alarmTime.get(Calendar.YEAR), alarmTime.get(Calendar.MONTH),
                    alarmTime.get(Calendar.DAY_OF_MONTH));
            Log.i("Calendar.YEAR==" + alarmTime.get(Calendar.YEAR) + "Calendar.MONTH=" + alarmTime.get(Calendar.MONTH) + "Calendar.DAY_OF_MONTH="
                    + alarmTime.get(Calendar.DAY_OF_MONTH) + " Calendar.DAY_OF_WEEK=" + alarmTime.get(Calendar.DAY_OF_WEEK));
            if ((alarmTime.get(Calendar.DAY_OF_WEEK) == Calendar.SUNDAY) || (alarmTime.get(Calendar.DAY_OF_WEEK) == Calendar.SATURDAY)) {
                if (!holidayMgr.isWorkday(alarmTime.get(Calendar.YEAR), alarmTime.get(Calendar.MONTH), alarmTime.get(Calendar.DAY_OF_MONTH))){
                    Log.i("isWeekend1----------");
                    isWeekend = true;
                }
            }
            ...
            while (isHoliday || isWeekend) {
                ...
                Log.i("Calendar.YEAR==" + alarmTime.get(Calendar.YEAR) + "Calendar.MONTH=" + alarmTime.get(Calendar.MONTH) + "Calendar.DAY_OF_MONTH="
                        + alarmTime.get(Calendar.DAY_OF_MONTH) + " Calendar.DAY_OF_WEEK=" + alarmTime.get(Calendar.DAY_OF_WEEK));
                isWeekend = false;
                isHoliday = holidayMgr.isHoliday(alarmTime.get(Calendar.YEAR), alarmTime.get(Calendar.MONTH), alarmTime.get(Calendar.DAY_OF_MONTH));
                if ((alarmTime.get(Calendar.DAY_OF_WEEK) == Calendar.SUNDAY) || (alarmTime.get(Calendar.DAY_OF_WEEK) == Calendar.SATURDAY)) {
                    if (!holidayMgr.isWorkday(alarmTime.get(Calendar.YEAR), alarmTime.get(Calendar.MONTH), alarmTime.get(Calendar.DAY_OF_MONTH))){
                        Log.i("isWeekend2----------");
                        isWeekend = true;
                    ...
                    int addDays = alarm.daysOfWeek.calculateDaysToNextAlarm(alarmTime);
                    if (addDays > 0) {
                        alarmTime.add(Calendar.DAY_OF_WEEK, addDays);
                    }
                    Log.v("addDays = "+addDays);
                    Log.v("fixAlarmInstances - alarmTime = "+alarmTime);
    ...
    private boolean isAlarmMissed(AlarmInstance instance) {
        Calendar alarmTime = instance.getAlarmTime();
        Calendar currentTime = Calendar.getInstance();
        Calendar alarmBuffer = Calendar.getInstance();
        alarmBuffer.setTime(alarmTime.getTime());
        ...
        if (currentTime.after(alarmBuffer)) {
            return true;
        } else {
            return false;
        }
    ...
    public void setAlarmState(Context context, AlarmInstance instance, int state) {
        switch(state) {
            ...
            case AlarmInstance.SNOOZE_STATE:
                //TODO penglei : this maybe cause snooze time error. test it hard
    ...
    public void onReceive(final Context context, final Intent intent) {
		Log.i("ClockShutdown,api:AlarmStateManager onReceive)");//liuqipeng add
...
//                return;
                //todo penglei : this cause the error, won't alarm
//            }
            Iterator it = null;
            String key = null;
            if(intent.getCategories()!=null){
                it = intent.getCategories().iterator();
            }
            if(it != null && it.hasNext()){
                key = it.next().toString();
            }
            if((alarmState == AlarmInstance.DISMISSED_STATE) && (key != null) && key.equals("DISMISS_TAG")){
                if((TA.getInstance()!=null)&&(TA.getInstance().getDefaultTracker()!=null)){
                    TA.getInstance().getDefaultTracker().commitEvent("Page_AlarmNotification",
                     2101, "CloseClock", null, null, null);
                }
            }
            if (alarmState >= 0) {
