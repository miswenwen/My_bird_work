...
public final class Alarm implements Parcelable, ClockContract.AlarmsColumns {
    ...
    public AlarmInstance createInstanceAfter(Calendar time,Context context) {
        ...
        if (daysOfWeek.mBitSet == daysOfWeek.LEGAL_DAYS) {
            HolidayManager holidayMgr = HolidayManager.getInstance(context);
            boolean isWeekend = false;
            boolean isHoliday = holidayMgr.isHoliday(nextInstanceTime.get(Calendar.YEAR), nextInstanceTime.get(Calendar.MONTH),
                    nextInstanceTime.get(Calendar.DAY_OF_MONTH));
            Log.i("Calendar.YEAR==" + nextInstanceTime.get(Calendar.YEAR) + " Calendar.MONTH=" + nextInstanceTime.get(Calendar.MONTH)
                    + " Calendar.DAY_OF_MONTH=" + nextInstanceTime.get(Calendar.DAY_OF_MONTH) + " Calendar.DAY_OF_WEEK="
                    + nextInstanceTime.get(Calendar.DAY_OF_WEEK));
            if ((nextInstanceTime.get(Calendar.DAY_OF_WEEK) == Calendar.SUNDAY) || (nextInstanceTime.get(Calendar.DAY_OF_WEEK) == Calendar.SATURDAY)) {
                if (!holidayMgr.isWorkday(nextInstanceTime.get(Calendar.YEAR), nextInstanceTime.get(Calendar.MONTH),
                        nextInstanceTime.get(Calendar.DAY_OF_MONTH))){
                    Log.i("isWeekend1----------");
                    isWeekend = true;
                }
            ...
            while (isHoliday || isWeekend) {
                Log.i("isBreakingHoliday----------");
                nextInstanceTime.add(Calendar.DAY_OF_YEAR, 1);
                Log.i("Calendar.YEAR==" + nextInstanceTime.get(Calendar.YEAR) + "Calendar.MONTH=" + nextInstanceTime.get(Calendar.MONTH)
                        + "Calendar.DAY_OF_MONTH=" + nextInstanceTime.get(Calendar.DAY_OF_MONTH) + " Calendar.DAY_OF_WEEK="
                        + nextInstanceTime.get(Calendar.DAY_OF_WEEK));
                isWeekend = false;
                isHoliday = holidayMgr.isHoliday(nextInstanceTime.get(Calendar.YEAR), nextInstanceTime.get(Calendar.MONTH),
                        nextInstanceTime.get(Calendar.DAY_OF_MONTH));
                if ((nextInstanceTime.get(Calendar.DAY_OF_WEEK) == Calendar.SUNDAY)
                        || (nextInstanceTime.get(Calendar.DAY_OF_WEEK) == Calendar.SATURDAY)) {
                    if (!holidayMgr.isWorkday(nextInstanceTime.get(Calendar.YEAR), nextInstanceTime.get(Calendar.MONTH),
                            nextInstanceTime.get(Calendar.DAY_OF_MONTH))){
                        Log.i("isWeekend2----------");
                        isWeekend = true;
        ...
        Log.i("nextInstanceTime3" + AlarmUtils.getFormattedTime(context, nextInstanceTime));
        ...
        Log.i("addDays1 = "+addDays);
        if (addDays > 0) {
            Log.i("Calendar.DAY_OF_WEEK1 = "+Calendar.DAY_OF_WEEK);
