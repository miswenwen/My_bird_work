...
public class TimerReceiver extends BroadcastReceiver {
    ...
    public void onReceive(final Context context, final Intent intent) {
        if (Timers.LOGGING) {
            Log.v(TAG, "Received intent " + intent.toString()+",  intent.getAction() = "+intent.getAction());
        }
        String actionType = intent.getAction();
        if (actionType.equals("android.intent.action.ACTION_PRE_SHUTDOWN")) {
            Log.v(TAG, "Received intent " + intent.getAction());
            SharedPreferences mPrefs =
                    PreferenceManager.getDefaultSharedPreferences(context);
            /*SharedPreferences.Editor editor = mPrefs.edit();
            if (mPrefs.getBoolean("isRunning",false)) {
                Log.d(TAG, "mPrefs.getBoolean = "+mPrefs.getBoolean("isRunning",false));
                editor.putBoolean("isPauseStatus", true);
                editor.putBoolean("isRunning", false);
                editor.apply();
            }*/
            Utils.sShutDown = true;
            TimerObj.resetTimersInSharedPrefs(mPrefs);
            Utils.clearSwSharedPref(mPrefs);
        }
        ...
        final TimerObj t = Timers.findTimer(mTimers, timerId);
        ...
        if (Timers.TIMES_UP.equals(actionType)) {
            ...
            //collapse the status bar if it is there
    ...
    private void updateNextTimesup(Context context) {
        ...
        if (t != null) {
            ...
            mngr.cancel(p);
            if (Timers.LOGGING) {
                Log.v(TAG, "no next times up timerId = "+timerId);
            }
        }
    }
    ...
    private static void showCollapsedNotification(final Context context, String title, String text,
        ...
        .setLargeIcon(BitmapFactory.decodeResource(context.getResources(),R.drawable.clock_timer_notification));
        if (showTicker) {
    ...
    private void showTimesUpNotification(final Context context, final TimerObj timerObj) {
