...
public final class DaysOfWeek implements Parcelable{
    ...
    // Value when work days are set
    public static final int ONE_TO_FIVE = 0x1f;

    // Value when weekend are set
    public static final int WEEKEND = 0x60;

    // Value when no days are set
    public static final int NO_DAYS_SET = 0;

    public static final int LEGAL_DAYS = 0xff;
    ...
    private String toString(Context context, boolean showNever, boolean forAccessibility) {
        ...
        if (mBitSet == NO_DAYS_SET) {
            return showNever ? context.getText(R.string.once).toString() : "";
        }
    ...
    public static final Parcelable.Creator<DaysOfWeek> CREATOR = new Parcelable.Creator<DaysOfWeek>() {
        public DaysOfWeek createFromParcel(Parcel p) {
            return new DaysOfWeek(p);
        }

        public DaysOfWeek[] newArray(int size) {
            return new DaysOfWeek[size];
        }
    };
