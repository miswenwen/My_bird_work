/*
 * Copyright (C) 2007 The Android Open Source Project
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *      http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */

package com.android.deskclock;

import android.content.BroadcastReceiver;
import android.content.ContentResolver;
import android.content.Context;
import android.content.Intent;
import android.content.SharedPreferences;
import android.os.PowerManager.WakeLock;
import android.preference.PreferenceManager;
import android.os.SystemProperties;

import com.android.deskclock.alarms.AlarmStateManager;
import com.android.deskclock.alarms.PowerOffAlarm;
import com.android.deskclock.provider.Alarm;
import com.android.deskclock.holiday.HolidayParser;

import com.android.deskclock.provider.AlarmInstance;
import com.android.deskclock.timer.TimerObj;
import java.util.Timer;
import java.util.TimerTask;
import java.util.Calendar;
import java.io.ByteArrayInputStream;
import java.io.InputStream;



public class AlarmInitReceiver extends BroadcastReceiver {

    // A flag that indicates that switching the volume button default was done
    private static final String PREF_VOLUME_DEF_DONE = "vol_def_done";
    ///M: Power off alarm for IPO boot@{
    private static final String IPO_BOOT_ACTION = "android.intent.action.ACTION_BOOT_IPO";
    public static final String PROP_POWERON_ALERT = "persist.env.deskclock.poalert";
    private static final String FIRST_ALARM_FLAG = "first_alarm";
    private static final String PROP_BOOT_MODE = "ro.alarm_boot";
    private static final String BOOT_ALARM = "true";
    private static Boolean sIPOBOOT = false;
    ///@}
    /**
     * Sets alarm on ACTION_BOOT_COMPLETED.  Resets alarm on
     * TIME_SET, TIMEZONE_CHANGED
     */
    @Override
    public void onReceive(final Context context, Intent intent) {
        final String action = intent.getAction();
        Log.v("AlarmInitReceiver " + action);


            if (HolidayProviderContract.ACTION_HOLIDAY_CHANGED.equals(action)) {
                int version = intent.getIntExtra(HolidayProviderContract.KEY_HOLIDAY_VERSION, -1);
                String dataStr = intent.getStringExtra(HolidayProviderContract.KEY_HOLIDAY_DATA);
                Log.e("intent Holiday: " + version + ", " + dataStr);
                if(version == -1){
                    DeskClock.mCalendarDataSupport = false;
                }
                else{
                    DeskClock.mCalendarDataSupport = true;
                }
                try {
                    InputStream inputStream = new ByteArrayInputStream(dataStr.getBytes());
                    DeskClock.parser = new HolidayParser();
                    DeskClock.parser.parse(inputStream);
                    DeskClock.parser.save(context, version);
                } catch (Exception e) {
                    Log.e("parseResourceFile(holiday) parse failed, delete ");

                }
                return;
            }

        if (PowerOffAlarm.bootFromPoweroffAlarm()) {
            return;
        }
        /* M: Power off alarm
         * Note: Never call updateNextAlarm when the device is boot from power off
         * alarm, since it would make the power off alarm dismiss the wrong alarm. @{
         */
        if (IPO_BOOT_ACTION.equals(action)) {
            Log.v("Receive android.intent.action.ACTION_BOOT_IPO intent.");
            setIPOBootValue(true);
            return;
        }
        //The TIME_SET broadcast is send by IPO, ACTION_TIME_CHANGED = TIME_SET
        if (getIPOBootValue() && action.equals(Intent.ACTION_TIME_CHANGED)) {
            Log.v("IPO boot time changed return");
            return;
        }
        if (("android.intent.action.TIME_SET").equals(action) || ("android.intent.action.LOCALE_CHANGED").equals(action)
                || ("android.intent.action.TIMEZONE_CHANGED").equals(action)) {
            Utils.sHideNotification = true;
            Utils.sChangByTime = true;
        } else {
            Utils.sHideNotification = false;
            Utils.sChangByTime = false;
        }
        ///@}

        final PendingResult result = goAsync();
        final WakeLock wl = AlarmAlertWakeLock.createPartialWakeLock(context);
        wl.acquire();

        // We need to increment the global id out of the async task to prevent
        // race conditions
        //AlarmStateManager.updateGloablIntentId(context);
        AsyncHandler.post(new Runnable() {
            @Override public void run() {
                // Remove the snooze alarm after a boot.
                if (action.equals(Intent.ACTION_BOOT_COMPLETED)) {
                    setIPOBootValue(false);
                    // Clear stopwatch and timers data
                    SharedPreferences prefs =
                            PreferenceManager.getDefaultSharedPreferences(context);
                    Log.v("AlarmInitReceiver - Reset timers and clear stopwatch data");
                    TimerObj.resetTimersInSharedPrefs(prefs);
                    Utils.clearSwSharedPref(prefs);
                    if (AlarmFeatureOption.YUNOS_MTK_PLATFORM) {
                        if (!prefs.getBoolean(PREF_VOLUME_DEF_DONE, false)) {
                            // Fix the default
                            Log.v("AlarmInitReceiver - resetting volume button default");
                            switchVolumeButtonDefault(prefs);
                        }
                    } else {
                        AlarmInstance nextAlarm = AlarmStateManager.getNearestAlarm(context);
                        Calendar calendar = Calendar.getInstance();
                        if (null != nextAlarm) {
                            Calendar alarmCalendar = nextAlarm.getAlarmTime();
                            long alarmTime = alarmCalendar.getTimeInMillis();
                            long nowTime = calendar.getTimeInMillis();
                            long diff = alarmTime - nowTime;
                            if (diff < 180000.0f && diff > 0.0f) {
                                saveFirstAlarm(prefs, true);
                                TimerTask task = new TimerTask(){
                                      public void run() {
                                        saveFirstAlarm(PreferenceManager.getDefaultSharedPreferences(context), false);
                                      }
                                };
                                Timer timer = new Timer();
                                timer.schedule(task, 3*60*1000);
                            }
                        }
                    }

                }

                // Update all the alarm instances on time change event
                AlarmStateManager.fixAlarmInstances(context);

                result.finish();
                Log.v("Utils.sHideNotification - finish = "+Utils.sHideNotification);
                Utils.sHideNotification = false;
                Log.v("AlarmInitReceiver finished");
                wl.release();
            }
        });
    }

    private void switchVolumeButtonDefault(SharedPreferences prefs) {
        SharedPreferences.Editor editor = prefs.edit();

        // editor.putString(SettingsActivity.KEY_VOLUME_BEHAVIOR,
        //     SettingsActivity.DEFAULT_VOLUME_BEHAVIOR);

        // Make sure we do it only once
        editor.putBoolean(PREF_VOLUME_DEF_DONE, true);
        editor.apply();
    }

     /**
     * Save flag for first Alarm
     */
    public static void saveFirstAlarm(SharedPreferences prefs,Boolean isFirstAlarm) {
        SharedPreferences.Editor editor = prefs.edit();
        editor.putBoolean(FIRST_ALARM_FLAG,isFirstAlarm);
        editor.apply();
    }

    ///M: get the value of IPO boot @{
    private boolean getIPOBootValue() {
        return sIPOBOOT;
    }
    ///@}

    ///M: set the value of IPO boot @{
    private void setIPOBootValue(boolean mValue) {
        sIPOBOOT = mValue;
    }
    ///@}
}
