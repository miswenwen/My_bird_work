...
import com.yunos.alicontacts.aliutil.provider.AliContactsContract.CommonDataKinds.Im;
...
public abstract class BaseAccountType extends AccountType {
    ...
    private interface Weight {
        ...
        static final int ORGANIZATION = 15;
        ...
        static final int SNS = 130; //added by xiaodong.lxd
        static final int SIP_ADDRESS = 140;
        static final int EVENT = 160;
        static final int RELATIONSHIP = 170;
    ...
    protected EditType buildSnsType(int type) {
        return new EditType(type, Sns.getProtocolLabelResource(type));
    }
...
//                R.string.name_phonetic_given, FLAGS_PHONETIC));
    ...
    protected DataKind addDataKindNickname(Context context) throws DefinitionException {
        DataKind kind = addKind(new DataKind(Nickname.CONTENT_ITEM_TYPE,
                    R.string.nicknameLabelsGroup, Weight.NICKNAME, true, R.layout.text_fields_editor_view));
        kind.typeOverallMax = 1;
    ...
    protected DataKind addDataKindPhone(Context context) throws DefinitionException {
        DataKind kind = addKind(new DataKind(Phone.CONTENT_ITEM_TYPE, R.string.phoneLabelsGroup,
                Weight.PHONE, true, R.layout.text_fields_editor_view));
        kind.iconAltRes = R.drawable.contacts_detail_message_button;
...
//        kind.typeList.add(buildEmailType(Email.TYPE_MOBILE));
    ...
    protected DataKind addDataKindStructuredPostal(Context context) throws DefinitionException {
        DataKind kind = addKind(new DataKind(StructuredPostal.CONTENT_ITEM_TYPE,
                R.string.postalLabelsGroup, Weight.STRUCTURED_POSTAL, true, R.layout.text_fields_editor_view));
...
//        kind.typeList.add(buildPostalType(StructuredPostal.TYPE_OTHER));
    ...
    protected DataKind addDataKindIm(Context context) throws DefinitionException {
        ...
        kind.typeList = Lists.newArrayList();
        kind.typeList.add(buildImType(Im.PROTOCOL_WANGWANG));
...
//        kind.typeList.add(buildImType(Im.PROTOCOL_JABBER));
    ...
    protected DataKind addDataKindNote(Context context) throws DefinitionException {
        DataKind kind = addKind(new DataKind(Note.CONTENT_ITEM_TYPE,
                    R.string.label_notes, Weight.NOTE, true, R.layout.text_fields_editor_view));
        kind.typeOverallMax = 1;
    ...
    protected DataKind addDataKindWebsite(Context context) throws DefinitionException {
        DataKind kind = addKind(new DataKind(Website.CONTENT_ITEM_TYPE,
                R.string.websiteLabelsGroup, Weight.WEBSITE, true, R.layout.text_fields_editor_view));
    ...
    //added by xiaodong.lxd
    protected DataKind addDataKindSns(Context context) throws DefinitionException {
        //Log.d(TAG, "sxsexe---> addDataKindSns ");
        DataKind kind = addKind(new DataKind(Sns.CONTENT_ITEM_TYPE,
                R.string.snsLabelsGroup, Weight.SNS, true, R.layout.text_fields_editor_view));
        kind.actionHeader = new SimpleInflater(R.string.snsLabelsGroup);
        kind.actionBody = new SimpleInflater(Sns.DATA);
        kind.defaultValues = new ContentValues();
        kind.defaultValues.put(Sns.TYPE, Sns.TYPE_OTHER);

        kind.typeColumn = Sns.COMMUNITY;
        kind.typeList = Lists.newArrayList();
        //Added by Zhangzujin for ali-SNS types
        kind.typeList.add(buildSnsType(Sns.COMMUNITY_RENREN));
        kind.typeList.add(buildSnsType(Sns.COMMUNITY_KAIXIN));
        kind.typeList.add(buildSnsType(Sns.COMMUNITY_MICROBLOG_SINA));
        kind.typeList.add(buildSnsType(Sns.COMMUNITY_CUSTOM).setSecondary(true).setCustomColumn(
                Sns.CUSTOM_COMMUNITY));

        kind.fieldList = Lists.newArrayList();
        kind.fieldList.add(new EditField(Sns.DATA, R.string.snsInputHint, FLAGS_EMAIL));
        return kind;
    }
    ...
    protected DataKind addDataKindGroupMembership(Context context) throws DefinitionException {
        DataKind kind = addKind(new DataKind(GroupMembership.CONTENT_ITEM_TYPE,
                R.string.groupsLabel, Weight.GROUP_MEMBERSHIP, true, -1));
    ...
    public static class ImActionInflater extends CommonInflater {
        ...
        protected int getTypeLabelResource(Integer type) {
            ...
            switch (type) {
                ...
                case Im.PROTOCOL_WANGWANG: return R.string.chat_wangwang;
    ...
    private static class ImKindBuilder extends KindBuilder {
        ...
        protected EditType buildEditTypeForTypeTag(AttributeSet attrs, String type) {
            ...
            if ("wangwang".equals(type)) return buildImType(Im.PROTOCOL_WANGWANG);
