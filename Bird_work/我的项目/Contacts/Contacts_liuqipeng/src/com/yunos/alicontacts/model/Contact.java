...
public class Contact {
    ...
    public Contact(Uri requestedUri, Uri uri, Uri lookupUri, long directoryId, String lookupKey,
                    ...
                    String photoUri, String displayName, String altDisplayName, String phoneticName,
                    boolean starred, Integer presence, boolean sendToVoicemail, String customRingtone,
                    boolean isUserProfile, boolean readonly) {
        this(requestedUri,uri,lookupUri,directoryId,lookupKey,
                id,nameRawContactId,displayNameSource,photoId,
                photoUri,displayName,altDisplayName,phoneticName,
                starred,presence,sendToVoicemail,customRingtone,
                isUserProfile);
        mReadOnly = readonly;
    ...
    public void setInvitableAccountTypes(ImmutableList<AccountType> accountTypes) {
        mInvitableAccountTypes = accountTypes;
    }

    public void setGroupMetaData(ImmutableList<GroupMetaData> groups) {
        mGroups = groups;
    }

    public void setStreamItems(ImmutableList<StreamItemEntry> streamItems) {
        mStreamItems = streamItems;
    }

    public ContentValues getSimContentValues(Context context) {
        if (mRawContacts == null || mRawContacts.size() == 0) {
            return null;
        }
        ...
        for (RawContact rawContact : mRawContacts) {
            ...
            for (DataItem dataItem : items) {
                ...
                if (dataItem instanceof StructuredNameDataItem) {
                    String displayName = ((StructuredNameDataItem) dataItem).getDisplayName();
                    if (!TextUtils.isEmpty(displayName)) {
                        values.put(SimUtil.SIM_NAME, displayName);
                    }

                } else if (dataItem instanceof PhoneDataItem) {
                    PhoneDataItem phoneData = (PhoneDataItem) dataItem;

                    String data = phoneData.getNumber();
                    if (!TextUtils.isEmpty(data)) {
                        int type = phoneData.getType();
                        if (type == Phone.TYPE_MOBILE) {
                            values.put(SimUtil.SIM_NUMBER, data);
                        } else {
                            values.put(SimUtil.SIM_ANR, data);
                        }
                    }

                } else if (dataItem instanceof EmailDataItem) {
                    String email = ((EmailDataItem) dataItem).getData();
    ...
    public boolean isSimAccountType() {
        return getSimAccountRawContact() != null;
    }

    public long getSimAccountRawContactId() {
        RawContact simContact = getSimAccountRawContact();
        if (simContact == null) {
            return -1;
        }
        return simContact.getId();
    }

    private RawContact getSimAccountRawContact() {
        if (mRawContacts == null || mRawContacts.size() == 0) {
            return null;
        }

        // In some cases, QQ (maybe other apps) might aggregate its contacts with a sim contact,
        // so the sim contact might be not the first raw contact in the contact.
        // We have to travel through all raw contacts and check the account type.
        // But for most of the time, we will break at the first round in loop.
        for (RawContact rawContact : mRawContacts) {
            if (rawContact != null
                    && SimContactUtils.SIM_ACCOUNT_TYPE.equals(rawContact.getAccountTypeString())) {
                return rawContact;
            }
        }
        return null;
    ...
    public int getSimSubscriptionBySimAccountName() {
        ...
        RawContact rawContact = mRawContacts.get(0);
        if (rawContact != null
                && SimContactUtils.SIM_ACCOUNT_NAME_SIM2.equals(rawContact.getAccountName())) {
