...
import com.yunos.alicontacts.model.account.AccountType.EventEditType;
...
public class EventFieldEditorView extends LabeledEditorView {
    ...
    public void setValues(DataKind kind, ValuesDelta entry, RawContactDelta state, boolean readOnly,
        ...
        //Log.d("EventFieldEditor", "sxsexe----> setValues ");
    ...
    private void rebuildDateView() {
        ...
        //setKindTypeImageVisible(true, getKind().mimeType);
    }
    ...
    private Dialog createDatePickerDialog() {
        ...
        if (TextUtils.isEmpty(oldValue)) {
            ...
            if (date1 != null) {
                ...
                // Unfortunately, we need a one-off hack for February 29th,
                // because
                // the parse functions assume 1970, which wasn't a leap year
                // Caveat here: This won't catch AccountTypes that allow
                // omitting the year but
                // require a time of the day. But as we don't have any of those
                // at the moment,
        ...
        if (mDatePickerDialog == null) {
            OnDateSetListener listener = new OnDateSetListener() {
                ...
                public void onDateSet(DatePicker view, int year, int monthOfYear, int dayOfMonth) {
                    Log.d(TAG, "OnDateSetListener.onDateSet: date selected.");
                    setDate(view, column, kind);
                }
            };
            mDatePickerDialog = new DatePickerDialog(getContext(), listener, oldYear, oldMonth, oldDay);
        } else {
            mDatePickerDialog.updateDate(oldYear, oldMonth, oldDay);
        }
        return mDatePickerDialog;
    }

    private void setDate(DatePicker dp, String column, DataKind kind) {
        int year = dp.getYear();
        int monthOfYear = dp.getMonth();
        int dayOfMonth = dp.getDayOfMonth();
        final Calendar outCalendar = Calendar.getInstance(DateUtils.UTC_TIMEZONE, Locale.US);

        // If no year specified, set it to 2000 (we could pick any
        // leap year here).
        // The format string will ignore that year.
        // For formats other than Exchange, the time of the day is
        // ignored
        outCalendar.clear();
        outCalendar.set(year, monthOfYear, dayOfMonth, DEFAULT_HOUR, 0, 0);
