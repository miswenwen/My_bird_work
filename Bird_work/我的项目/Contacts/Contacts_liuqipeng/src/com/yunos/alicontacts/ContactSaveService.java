...
import com.yunos.alicontacts.util.CallerInfoCacheUtils;
...
public class ContactSaveService extends IntentService {
    ...
    private static final boolean PERFORMANCE_DBG = true;
    ...
    public static final String EXTRA_OPERATE_GROUP_RESULT = "operateGroupResult";
    ...
    public static final String EXTRA_HAS_STARRED_FLAG = "hasStarred";
    public static final String EXTRA_SIM_RAWCONTACT_ID = "rawContactId";
    ...
    public static final String EXTRA_SAVE_RINGTONE = "needSaveRingtone";
    ...
    protected void onHandleIntent(Intent intent) {
        ...
        try {
            ...
            showToast(R.string.toast_phone_storage_insufficient);
            Log.e(TAG, "onHandleIntent: storage full.", sfe);
        }
    }
    ...
    private void createRawContact(Intent intent) {
        ...
        if (accountName == null && accountType == null && dataSet == null) {
            accountName = AccountType.LOCAL_ACCOUNT_NAME;
            accountType = AccountType.LOCAL_ACCOUNT_TYPE;
        }
...
//    }
    ...
    public static Intent createSaveContactIntent(Context context, RawContactDeltaList state,
            String saveModeExtraKey, int saveMode, boolean isProfile,
            Class<? extends Activity> callbackActivity, String callbackAction, long rawContactId,
            Uri updatedPhotoPath, String ringtoneUri, boolean saveRingtone) {
        Bundle bundle = new Bundle();
        bundle.putParcelable(String.valueOf(rawContactId), updatedPhotoPath);
        return createSaveContactIntent(context, state, saveModeExtraKey, saveMode, isProfile,
                callbackActivity, callbackAction, bundle, ringtoneUri, saveRingtone);
    }
    ...
    public static Intent createSaveContactIntent(Context context, RawContactDeltaList state,
            String saveModeExtraKey, int saveMode, boolean isProfile,
            Class<? extends Activity> callbackActivity, String callbackAction,
            Bundle updatedPhotos, String ringtoneUri, boolean saveRingtone) {
        Intent serviceIntent = new Intent(
                context, ContactSaveService.class);
        ...
        serviceIntent.putExtra(ContactSaveService.EXTRA_SAVE_RINGTONE, saveRingtone);
        if (saveRingtone) {
            serviceIntent.putExtra(ContactSaveService.EXTRA_CUSTOM_RINGTONE, ringtoneUri);
        }
    ...
    private void saveContact(Intent intent) {
        if(PERFORMANCE_DBG) Log.d(PERFORMANCE_TAG, "saveContact start");
        ...
        while (tries++ < PERSIST_TRIES) {
            try {
                ...
                // YunOS contact will not do save action when join contact, so rawContactId in updatedPhotos key
                // will < 0 when create new contact, will have no chance to change the id to > 0 value.
                // But the rawContactId is the contactId we save data to, so use it is ok.
                ...
                Log.e(TAG, "Cannot operate on Database, the reason is "+sqe.getMessage(), sqe);
                if (tries == (PERSIST_TRIES - 1)) {
        ...
        if (succeeded && (updatedPhotos != null)) {
...
//        boolean hasStarred = intent.getBooleanExtra(EXTRA_HAS_STARRED_FLAG, false);
//        if (hasStarred && lookupUri != null) {
//          boolean starred = intent.getBooleanExtra(EXTRA_STARRED_FLAG, false);
...
//        }
        ...
        // Save ringtone
        boolean needSaveRingtone = succeeded && intent.getBooleanExtra(ContactSaveService.EXTRA_SAVE_RINGTONE, false);
        if (needSaveRingtone) {
        ...
        if (callbackIntent != null) {
            ...
            callbackIntent.putExtra(EXTRA_SAVE_SUCCEEDED, succeeded);
            callbackIntent.setData(lookupUri);
            if(PERFORMANCE_DBG) Log.d(PERFORMANCE_TAG, "saveContact deliverCallback start");
            // Here will call the callBack to do some thing finally, but the callback should not do UI options
            // or maybe crash will happen.
            callbackIntent.putExtra(ContactDetailActivity.EXTRA_DETAIL_DATA, (Parcelable) state);
            callbackIntent.putExtra(ContactDetailActivity.EXTRA_DETAIL_IS_PROFILE, isProfile);
            deliverCallbackOnUiThread(callbackIntent);
            int saveMode = callbackIntent.getIntExtra(ContactEditorFragment.SAVE_MODE_EXTRA_KEY, SaveMode.CLOSE);
            if (succeeded) {
    ...
    private boolean saveUpdatedPhoto(long rawContactId, Uri photoUri) {
        if(DEBUG) {
            Log.v(TAG, "saveUpdatedPhoto(), url = " + photoUri + ", rawContactId = " + rawContactId);
        }
        if (photoUri == null) {
            try {
                // If the user set a pre-defined photo to the contact and set back to default photo again,
                // then we still can get here, and get 0 for delete result.
                // So we can not use the return value of delete to judge success.
                getContentResolver().delete(
                        Data.CONTENT_URI,
                        Data.MIMETYPE + "=? AND " + Data.RAW_CONTACT_ID + "=?",
                        new String[] { Photo.CONTENT_ITEM_TYPE, String.valueOf(rawContactId) });
            } catch (SQLiteException sqle) {
                Log.e(TAG, "saveUpdatedPhoto: got exception in delete photo.", sqle);
                return false;
            }
            return true;
        }
     ...
     */
    private long getInsertedRawContactId(final ArrayList<ContentProviderOperation> diff,
            final ContentProviderResult[] results) {
        final int diffSize = diff.size();

        // Monkey crash : BugID -- 140104
        // Maybe results have different size with "diff" operations list
        if (results == null || diffSize != results.length) {
            Log.i(TAG, "[LongTermTrackingLog] getInsertedRawContactId: return -1 for"
                        + " results size=" + (results == null ? -1 : results.length)
                        + "; diff size=" + diffSize);
            return -1;
        }

        for (int i = 0; i < diffSize; i++) {
            // Merge Android 4.4.4-r1 code
        ...
        Log.i(TAG, "[LongTermTrackingLog] getInsertedRawContactId: return -1 for not found.");
        return -1;
    }
     ...
     */
    public static Intent createNewGroupIntent(Context context, AccountWithDataSet account, String label,
            long[] rawContactsToAdd, Class<? extends Activity> callbackActivity, String callbackAction) {
    ...
    private void createGroup(Intent intent) {
        ...
        if (accountType == null && accountName == null && dataSet == null) {
            accountType = AccountType.LOCAL_ACCOUNT_TYPE;
            accountName = AccountType.LOCAL_ACCOUNT_NAME;
        }
        ...
        Intent callbackIntent = intent.getParcelableExtra(EXTRA_CALLBACK_INTENT);
        if (callbackIntent != null) {
            callbackIntent.putExtra(EXTRA_OPERATE_GROUP_RESULT, (groupUri != null));
    ...
    private void renameGroup(Intent intent) {
        ...
        int result = getContentResolver().update(groupUri, values, null, null);

        Intent callbackIntent = intent.getParcelableExtra(EXTRA_CALLBACK_INTENT);
        if (callbackIntent != null) {
            callbackIntent.putExtra(EXTRA_OPERATE_GROUP_RESULT, result > 0);
            callbackIntent.setData(groupUri);
            deliverCallback(callbackIntent);
        }
    ...
    private void deleteGroup(Intent intent) {
        ...
        if (result > 0) {
            Intent callbackIntent = intent.getParcelableExtra(EXTRA_CALLBACK_INTENT);
            Log.d(PERFORMANCE_TAG, "deleteGroup intent:" + callbackIntent);
            if (callbackIntent != null) {
                callbackIntent.putExtra(EXTRA_OPERATE_GROUP_RESULT, true);
                deliverCallback(callbackIntent);
            }
        }
    }
        ...
        for (long rawContactId : rawContactsToAdd) {
            try {
                ...
                Log.w(TAG, "Assert failed in adding raw contact ID " +
                        rawContactId + ". Already exists in group " +
                        groupId, e);
            }
        }
    }
    ...
    public static Intent createDeleteContactIntent(Context context, Uri contactUri,
            long simRawContactId) {
        Intent serviceIntent = new Intent(context, ContactSaveService.class);
        serviceIntent.setAction(ContactSaveService.ACTION_DELETE_CONTACT);
        serviceIntent.putExtra(ContactSaveService.EXTRA_CONTACT_URI, contactUri);
        serviceIntent.putExtra(ContactSaveService.EXTRA_SIM_RAWCONTACT_ID, simRawContactId);
        return serviceIntent;
    }
    ...
    private void deleteContact(Intent intent) {
        ...
        final long rawContactId = intent.getLongExtra(EXTRA_SIM_RAWCONTACT_ID, 0);
        final ContentResolver resolver = getContentResolver();

        /* if rawContactId is not equal 0, delete sim card records first. */
        if (rawContactId != 0) {
            Log.d(TAG, "deleteContact() delete SIM records, rawContactId:" + rawContactId);
            ...
            if (simContact != null) {
                ...
                if (SimUtil.MULTISIM_ENABLE) {
                    ...
                    result = SimUtil.delete(this, slotId, values);
                    Log.d(TAG, "deleteContact() slotId:" + slotId + ", values[" + values + "], result:" + result);
                } else {
                    result = SimUtil.delete(this, values);
                    Log.d(TAG, "deleteContact() mValues[" + values + "], result:" + result);
                }
                ...
                if (result > 0) {
                    ...
                    showToast(R.string.delete_sim_contact_failed);
                }
                return;
            } else {
                Log.e(TAG, "deleteContact() delete SIM records fail!!!");
            }
...
//        boolean writable = intent.getBooleanExtra(EXTRA_CONTACT_WRITABLE, false);
        ...
        if (c == null || c.getCount() == 0) {
            Log.e(TAG, "Invalid arguments c for joinContacts request");
            if (c != null) {
                c.close();
            }
            return;
...
//                }
        ...
        Uri uri;
        if (success) {
            uri = RawContacts.getContactLookupUri(resolver,
                    ContentUris.withAppendedId(RawContacts.CONTENT_URI, rawContactIds[0]));
            callbackIntent.setData(uri);
            callbackIntent.setAction(ContactSaveService.ACTION_JOIN_CONTACTS);
        }
    ...
    private void deliverCallback(final Intent callbackIntent) {
        mMainHandler.post(new Runnable() {
            ...
            @Override
            public void run() {
                if(PERFORMANCE_DBG) Log.d(PERFORMANCE_TAG, "saveContact deliverCallback run deliverCallbackOnUiThread");
                deliverCallbackOnUiThread(callbackIntent);
            }
        });
    }
