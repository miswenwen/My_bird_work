...
public class ExternalAccountType extends BaseAccountType {
     ...
     */
    private XmlResourceParser loadContactsXml(Context context, String resPackageName) throws NameNotFoundException {
        final PackageManager pm = context.getPackageManager();
        PackageInfo packageInfo = pm.getPackageInfo(resPackageName, PackageManager.GET_SERVICES | PackageManager.GET_META_DATA);
            ...
            for (String metadataName : METADATA_CONTACTS_NAMES) {
                ...
                if (parser != null) {
                    if (Log.isLoggable(TAG, Log.DEBUG)) {
                        Log.d(TAG, String.format("Metadata loaded from: %s, %s, %s", serviceInfo.packageName, serviceInfo.name,
                                metadataName));
                    }
                    return parser;
                }
            ...
            while (((type = parser.next()) != XmlPullParser.END_TAG
                ...
                if (TAG_EDIT_SCHEMA.equals(tag)) {
                    ...
                    final boolean detailSocialSummary = a.getBoolean(
                            android.R.styleable.ContactsDataKind_detailSocialSummary,
                            false);
                    ...
                    if (!TextUtils.isEmpty(kind.mimeType)) {
                        addKind(kind);
                    }
                }
            }
