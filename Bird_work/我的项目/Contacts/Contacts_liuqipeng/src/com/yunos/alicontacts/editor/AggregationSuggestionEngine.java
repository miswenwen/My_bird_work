...
public class AggregationSuggestionEngine extends HandlerThread {
    ...
    public AggregationSuggestionEngine(Context context) {
        ...
        mMainHandler = new Handler() {
            ...
            public void handleMessage(Message msg) {
                ...
                if (!mQuited) {
                    AggregationSuggestionEngine.this.deliverNotification(cursor);
                } else {
                    if (cursor != null) {
                        cursor.close();
                    }
    ...
    protected void scheduleSuggestionLookup() {
        ...
        if (mSuggestionsUri == null) {
            reset();
            mMainHandler.sendMessage(mMainHandler.obtainMessage(MESSAGE_DATA_CURSOR, null));
            return;
        }
    ...
    private Uri buildAggregationSuggestionUri(ValuesDelta values) {
        ...
        if (nameSb.length() != 0) {
            PDUtils.addNameToAggregationSuggestionsBuilder(builder, nameSb.toString());
        }

        if (phoneticNameSb.length() != 0) {
            PDUtils.addNameToAggregationSuggestionsBuilder(builder, phoneticNameSb.toString());
        }
    ...
    private void loadAggregationSuggestions(Uri uri) {
        ...
        Cursor cursor = null;
        try {
            cursor = contentResolver.query(uri, new String[]{Contacts._ID}, where, null, null);
        } catch (IllegalStateException e) {
            Log.e(TAG, "loadAggregationSuggestions IllegalStateException", e);
            return;
        }
        if (cursor == null) {
            Log.e(TAG, "loadAggregationSuggestions, error, cursor is null!!!");
            return;
        }
        try {
