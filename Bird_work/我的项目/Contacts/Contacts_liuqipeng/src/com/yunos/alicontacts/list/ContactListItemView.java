...
import com.yunos.alicontacts.ContactStatusUtil;
import com.yunos.alicontacts.R;
import com.yunos.alicontacts.format.PrefixHighlighter;
...
//                R.styleable.ContactListItemView_list_item_divider);
        ...
        final int prefixHighlightColor = a.getColor(
                R.styleable.ContactListItemView_list_item_prefix_highlight_color, Color.RED);
        mPrefixHighlighter = new PrefixHighlighter(prefixHighlightColor);
        a.recycle();

        a = getContext().obtainStyledAttributes(R.styleable.Theme);
        mSecondaryTextColor = a.getColorStateList(R.styleable.Theme_textColorSecondary);
        a.recycle();

        mHorizontalDividerHeight = 4;//mHorizontalDividerDrawable.getIntrinsicHeight();
...
//            mHorizontalDividerDrawable.draw(canvas);
    ...
    public TextView getCountView() {
        if (mCountView == null) {
            ...
            mCountView.setTextColor(getResources().getColor(R.color.contact_count_text_color));
            addView(mCountView);
        }
        return mCountView;
    }
    ...
    public void setStarred(Drawable icon) {
        if (icon != null) {
            if (mStarredIcon == null) {
            	mStarredIcon = new ImageView(mContext);
                addView(mStarredIcon);
            }
            mStarredIcon.setImageDrawable(icon);
            mStarredIcon.setScaleType(ScaleType.CENTER);
            mStarredIcon.setVisibility(View.VISIBLE);
        } else {
            if (mStarredIcon != null) {
            	mStarredIcon.setVisibility(View.GONE);
            }
        }
    }
    ...
    public void showStarred(Cursor cursor, int starredColumnIndex) {
        Drawable icon = null;
        int starred = 0;
        if (!cursor.isNull(starredColumnIndex)) {
        	starred = cursor.getInt(starredColumnIndex);
        	if(1 == starred) {
        		icon = getContext().getResources().getDrawable(
        				R.drawable.ic_ab_favourites_holo_dark);
        	}
        }
        setStarred(icon);
    }
    ...
    public void showPresenceAndStatusMessage(Cursor cursor, int presenceColumnIndex,
        ...
        icon = ContactPresenceIconUtil.getPresenceIcon(getContext(), 1);
        presence = 1;
        setPresence(icon);
