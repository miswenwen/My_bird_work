...
public abstract class ContactBrowseListFragment extends
    ...
    /**
     * This is an indicator to tell if there is a long period db operation on contacts list.
     * It is also used as wait/notify object for synchronize pause in the db operation.
     */
    public static final AtomicBoolean CONTACTS_LIST_LONG_OPERATION = new AtomicBoolean(false);
    ...
    public void onLoadFinished(Loader<Cursor> loader, Cursor data) {
        ...
        if (CONTACTS_LIST_LONG_OPERATION.get()) {
            synchronized (CONTACTS_LIST_LONG_OPERATION) {
                CONTACTS_LIST_LONG_OPERATION.notifyAll();
            }
        }
        mSelectionVerified = false;
    ...
    public void viewContact(Uri contactUri, boolean isSimContact) {
        setSelectedContactUri(contactUri, false, false, true, false);
        if (mListener != null) mListener.onViewContactAction(contactUri, isSimContact);
    }

    public void editPhoneContact(Uri contactUri) {
