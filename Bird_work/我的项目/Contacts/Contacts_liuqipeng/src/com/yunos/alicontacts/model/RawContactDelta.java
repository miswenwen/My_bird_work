...
package com.yunos.alicontacts.model;
 ...
 * empty {@link RawContact}, which then becomes an insert {@link RawContacts}
 * case.
 * <p>
 ...
 * rows are missing from the new {@link RawContact}, we know the original data
 * must be deleted, but to preserve the user modifications we treat as an
 * insert.
 */
public class RawContactDelta implements Parcelable {
    // TODO: optimize by using contentvalues pool, since we allocate so many of
    // them

    private static final String TAG = "EntityDelta";
    private static final boolean LOGV = false;
    private static final int NEED_ADD_YIELD_POINT_SIZE = 200;
    ...
    /**
     * URI used for contacts queries, by default it is set to query raw
     * contacts. It can be set to query the profile's raw contact(s).
     */
     ...
     * NOTE By YunOS: when we buildDiff for saveContact, we want the name entries to be at first,
     * so that we can have name record before phone number record in the data table.
     * If in the data table, name record appears before phone number record,
     * then WeiXin will create multiple raw_contacts for the same phone number.
     */
    private final TreeMap<String, ArrayList<ValuesDelta>> mEntries
            = new TreeMap<String, ArrayList<ValuesDelta>>(new Comparator<String>() {
        private static final String NAME_MIMETYPE = StructuredName.CONTENT_ITEM_TYPE;
        /*
         * null is the smallest, then NAME_MIMETYPE, then others.
         */
        @Override
        public int compare(String lhs, String rhs) {
            ...
            if (NAME_MIMETYPE.equals(lhs)) {
                if (NAME_MIMETYPE.equals(rhs)) {
                    return 0;
                }
                return -1;
            } else if (NAME_MIMETYPE.equals(rhs)) {
                return 1;
            }
     ...
     * "before" state represented by this {@link RawContactDelta}, discarding
     * any existing "after" states. This is typically used when re-parenting
     * changes onto an updated {@link Entity}.
     */
    ...
    public ValuesDelta getPrimaryEntry(String mimeType) {
        ...
        return mimeEntries.isEmpty() ? null : mimeEntries.get(0) ;
    }
     ...
     *
     * @param forceSelection if true, will try to return some value even if a
     *            super-primary doesn't exist (may be a primary, or just a
     *            random item
     * @return
     */
    @NeededForTesting
    ...
    public boolean equals(Object object) {
        if (object instanceof RawContactDelta) {
            ...
            if (!other.mValues.equals(mValues))
                return false;
    ...
    private void possibleAdd(ArrayList<ContentProviderOperation> diff,
        ...
        if (builder != null) {
            checkNeedYieldAndAdd(diff, builder, -1);
        }
    }
    ...
    public void buildAssert(ArrayList<ContentProviderOperation> buildInto) {
        ...
        if (!isContactInsert) {
            ...
            checkNeedYieldAndAdd(buildInto, builder, -1);
        }
    }

    private void checkNeedYieldAndAdd(
            ArrayList<ContentProviderOperation> buildInto,
            ContentProviderOperation.Builder builder,
            int addIndex) {
        int size = buildInto.size();
        if (size % NEED_ADD_YIELD_POINT_SIZE == 0 && size != 0) {
            builder.withYieldAllowed(true);
        }
        if (addIndex > -1) {
            buildInto.add(addIndex, builder.build());
        } else {
    ...
    public void buildDiff(ArrayList<ContentProviderOperation> buildInto) {
        ...
        if (isContactInsert) {
            ...
            // created on the phone. In the future, will show aggregation
            // suggestions
        ...
        for (ArrayList<ValuesDelta> mimeEntries : mEntries.values()) {
            for (ValuesDelta child : mimeEntries) {
                ...
                // Bug:105022 begin
                if (child.isInsert() && TextUtils.isEmpty(child.getMimetype())) {
                    Log.w(TAG,
                            "[LongTermTrackingLog] Mimetype is NULL when build Operation for child entry :  "
                                    + child.toString());
                    continue;
                }
                // Bug:105022 end
        ...
        if (addedOperations && isContactUpdate) {
            ...
            checkNeedYieldAndAdd(buildInto, builder, firstIndex);
    ...
    public static class ValuesDelta implements Parcelable {
        ...
        /**
         * Create {@link ValuesDelta}, using the given object as the "before"
         * state, usually from an {@link Entity}.
         */
        ...
        public boolean containsKey(String key) {
            return ((mAfter != null && mAfter.containsKey(key)) || (mBefore != null && mBefore
                    .containsKey(key)));
        }
        ...
        public static ValuesDelta mergeAfter(ValuesDelta local, ValuesDelta remote) {
            ...
            if (!local.beforeExists()) {
                // Any "before" record is missing, so take all values as
                // "insert"
        ...
        public int hashCode() {
            ...
            for (String key : keySet()) {
                final String value = this.getAsString(key);
