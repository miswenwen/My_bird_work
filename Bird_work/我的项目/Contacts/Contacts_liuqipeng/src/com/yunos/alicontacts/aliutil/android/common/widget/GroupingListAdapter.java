...
package com.yunos.alicontacts.aliutil.android.common.widget;
...
/**
 * Maintains a list that groups adjacent items sharing the same value of a
 * "group-by" field. The list has three types of elements: stand-alone, group
 * header and group child. Groups are collapsible and collapsed by default.
 */
public abstract class GroupingListAdapter extends BaseAdapter {
    ...
    /**
     * Information about a specific list item: is it a group, if so is it
     * expanded. Otherwise, is it a stand-alone item or a group member.
     */
    ...
    /**
     * Information about where these groups are located in the list, how large
     * they are and whether they are expanded.
     */
     ...
     * {@link #addGroup} for each of them.
     */
    ...
    protected abstract void bindChildView(View view, Context context, Cursor cursor, int position);

    /**
     * Cache should be reset whenever the cursor changes or groups are expanded
     * or collapsed.
     */
    ...
    public void changeCursor(Cursor cursor) {
        AliCallLogExtensionHelper.log("GroupingListAdapter", "sxsexe ---->changeCursor  cursor "
    ...
    /**
     * Scans over the entire cursor looking for duplicate phone numbers that
    ...
    /**
     * Records information about grouping in the list. Should be called by the
     * overridden {@link #addGroups} method.
     */
    ...
    public void obtainPositionMetadata(PositionMetadata metadata, int position) {
        ...
        // If the description object already contains requested information,
        // just return
        ...
        // is the header of the group, a member of the group or a standalone
        // item.
        if (mLastCachedListPosition != -1) {
            if (position <= mLastCachedListPosition) {
                ...
                if (index < 0) {
                    ...
                    // We had a cache miss, but we did obtain valuable
                    // information anyway.
                    // The negative number will allow us to compute the location
                    // of
                    // the group header immediately preceding the supplied
                    // position.
                    index = ~index - 1;
                ...
                // A non-negative index gives us the position of the group
                // header
    ...
    /**
     * Returns true if the specified position in the list corresponds to a group
     * header.
     */
    ...
    /**
     * Given a position of a groups header in the list, returns the size of the
     * corresponding group.
     */
        ...
        switch (mPositionMetadata.itemType) {
            ...
            case ITEM_TYPE_GROUP_HEADER:
                bindGroupView(view, mContext, mCursor, mPositionMetadata.childCount,
                        mPositionMetadata.isExpanded, position);
                break;
            case ITEM_TYPE_IN_GROUP:
