...
public class ImageViewDrawableSetter {
    ...
    public void setupContactPhoto(Contact contactData, ImageView photoView) {
        setTarget(photoView);
        setCompressedImage(contactData);
    }
	...
	protected Bitmap setCompressedImage(byte[] compressed) {
        ...
        Drawable newDrawable = (compressed == null)
                ? defaultDrawable()
                : decodedBitmapDrawable(compressed);

        newDrawable = new BitmapDrawable(BitmapUtil.circleMaskBitmap(
                ((BitmapDrawable) newDrawable).getBitmap()));
    ...
    protected Bitmap setCompressedImage(Contact contact) {
        byte[] compressed = contact.getPhotoBinaryData();
        ...
        if (compressed == null) {
            if (contact.isSimAccountType()) {
                int subscription = contact.getSimSubscriptionBySimAccountName();
                newDrawable = simCardDrawable(subscription);
            } else {
    ...
    private Drawable simCardDrawable(int subscription) {
        Resources resources = mTarget.getResources();
        try {
            if (SimUtil.MULTISIM_ENABLE) {
                return resources
                        .getDrawable((subscription == SimUtil.SLOT_ID_1) ? R.drawable.contact_detail_avatar_border_acquiesce_card1
                                : R.drawable.contact_detail_avatar_border_acquiesce_card2);
            } else {
                return resources
                        .getDrawable(R.drawable.contact_detail_avatar_border_acquiesce_sim);
            }
