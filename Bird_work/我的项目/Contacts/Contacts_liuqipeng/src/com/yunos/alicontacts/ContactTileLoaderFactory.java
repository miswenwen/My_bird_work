...
import com.yunos.alicontacts.list.ContactTileView;
//add by ali.xgy
...
public final class ContactTileLoaderFactory {
    ...
    /*
     * To get frequent contacts in phone memory (not in SIM card).
     */
    public static CursorLoader createStrequentInPhoneLoader(Context context) {
    	if (SimUtil.IS_YUNOS) {
            return new CursorLoader(context, Contacts.CONTENT_STREQUENT_URI, COLUMNS, null, null, null);
    	} else {
            return new CursorLoader(context, Contacts.CONTENT_STREQUENT_URI, COLUMNS, null, null, null);
    ...
    public static CursorLoader createStarredLoader(Context context) {
        return new CursorLoader(context, Contacts.CONTENT_URI, COLUMNS,
                Contacts.STARRED + "=?", new String[]{"1"}, Contacts.SORT_KEY_PRIMARY);
    }

    public static CursorLoader createFrequentLoader(Context context) {
        return new CursorLoader(context, ContactsContract.Contacts.CONTENT_FREQUENT_URI, COLUMNS,
