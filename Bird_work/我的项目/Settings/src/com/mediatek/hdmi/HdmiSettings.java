...
public class HdmiSettings extends SettingsPreferenceFragment implements
    ...
    private BroadcastReceiver mActionReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (DisplayManager.ACTION_WIFI_DISPLAY_STATUS_CHANGED.equals(action)
                    && Settings.Global.getInt(mActivity.getContentResolver(),
                    Settings.Global.WIFI_DISPLAY_ON, 0) == 0 && mRet) {
                Log.d(TAG, "wifi display disconnected");
                try {
    ...
    private void showDialog() {
        ...
        String messageString = mActivity.getResources().getString(R.string.hdmi_wfd_off_hdmi_on);
        // HDMI excluded dialog, HDMI update
        mHDMIExcludeDialog = new AlertDialog.Builder(mContext)
          .setMessage(messageString)
          .setPositiveButton(mResource.getString(com.android.internal.R.string.dlg_ok),
              new OnClickListener() {
              @Override
              public void onClick(DialogInterface dialog, int which) {
                  Log.d(TAG, "HDMI on, turn off WifiDisplay");
                  Settings.Global.putInt(mContext.getContentResolver(),
                    Settings.Global.WIFI_DISPLAY_ON, 0);
                  mRet = true;
             }
          })
          .setNegativeButton(mResource.getString(com.android.internal.R.string.decline),
              new OnClickListener() {
              @Override
              public void onClick(DialogInterface dialog, int which) {
                  Log.d(TAG, "HDMI on, user DON'T turn off WifiDisplay -> turn off HDMI");
                  mRet = false;
                  updatePref();
              }
          })
          .setOnCancelListener(new DialogInterface.OnCancelListener() {
              ...
              public void onCancel(DialogInterface arg0) {
                  Log.d(TAG, "onCancel(): user DON'T turn off WifiDisplay -> turn off HDMI");
                  mRet = false;
                  updatePref();
              }
          })
          ...
          .create();
        popupDialog(mHDMIExcludeDialog);
    }
    @Override
    ...
    public boolean onPreferenceChange(Preference preference, Object newValue) {
        ...
        try {
            if (KEY_TOGGLE_HDMI.equals(key)) {
                ...
                Log.d("@M_" + TAG, key + " enableHdmi start");
