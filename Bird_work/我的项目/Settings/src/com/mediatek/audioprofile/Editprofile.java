...
public class Editprofile extends SettingsPreferenceFragment {
    ...
    // @ { BIRD zhouleigang 20141020, BIRD_AUDIO_CLAM_RING begin
    private static final String KEY_FLIP_SOUNDS = "flip_sounds";
    private TwoStatePreference mFlipSounds;
    // @ }
    
    // @ { BIRD jiali 20160815, BIRD_FLIP_ANSWER_CALL_MENU begin
    private static final String KEY_FLIP_ANSWER_CALL = "flip_answer_call";
    private TwoStatePreference mFlipAnswerCall;
    // @ }
    ...
    private void initSystemAudio() {
        ...
        // @ { BIRD zhouleigang 20141020, BIRD_AUDIO_CLAM_RING begin
        mFlipSounds = (TwoStatePreference) findPreference(KEY_FLIP_SOUNDS);
        if (BirdFeatureOption.BIRD_AUDIO_CLAM_RING) {
            Boolean flip = Settings.System.getInt(getContentResolver(), Settings.System.BIRD_FLIP_CHANGE_SOUND, 1) == 1;
            mFlipSounds.setChecked(flip);
        }
        // @ }

        // @ { BIRD zhouleigang 20141020, BIRD_AUDIO_CLAM_RING begin
        if (!BirdFeatureOption.BIRD_AUDIO_CLAM_RING || mIsSilentMode || mIsMeetingMode) {
            PreferenceScreen parent = getPreferenceScreen();
            parent.removePreference(mFlipSounds);
        }
		// @ }
        
        // @ { BIRD jiali 20160815, BIRD_FLIP_ANSWER_CALL_MENU begin
        mFlipAnswerCall = (TwoStatePreference) findPreference(KEY_FLIP_ANSWER_CALL);
        if (BirdFeatureOption.BIRD_FLIP_ANSWER_CALL_MENU) {
            Boolean flip = Settings.System.getInt(getContentResolver(), Settings.System.BIRD_FLIP_ANSWER_CALL, 1) == 1;
            mFlipAnswerCall.setChecked(flip);
        }
        // @ }

        // @ { BIRD jiali 20160815, BIRD_FLIP_ANSWER_CALL_MENU begin
        if (!BirdFeatureOption.BIRD_FLIP_ANSWER_CALL_MENU || mIsSilentMode || mIsMeetingMode) {
            PreferenceScreen parent = getPreferenceScreen();
            parent.removePreference(mFlipAnswerCall);
        }
		// @ }
    }
    ...
    public boolean onPreferenceTreeClick(PreferenceScreen preferenceScreen,
        ...
        // @ { BIRD zhouleigang 20141020, BIRD_AUDIO_CLAM_RING begin
        } else if (BirdFeatureOption.BIRD_AUDIO_CLAM_RING && (preference.getKey()).equals(KEY_FLIP_SOUNDS)) {         
            Settings.System.putInt(getContentResolver(),Settings.System.BIRD_FLIP_CHANGE_SOUND, mFlipSounds.isChecked() ? 1 : 0);	
        // @ }
        
        
        // @ { BIRD jiali 20160815, BIRD_FLIP_ANSWER_CALL_MENU begin
        } else if (BirdFeatureOption.BIRD_FLIP_ANSWER_CALL_MENU && (preference.getKey()).equals(KEY_FLIP_ANSWER_CALL)) {         
            Settings.System.putInt(getContentResolver(),Settings.System.BIRD_FLIP_ANSWER_CALL, mFlipAnswerCall.isChecked() ? 1 : 0);	
        // @ }
        }
