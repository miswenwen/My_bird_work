...
// @{ BIRD_SYSTEM_SPACE, add by shenzhiwang, @20160707
    private static final int[] BIRD_ITEMS_SHOW_SHARED = new int[] {
            R.string.storage_detail_systemspace,
            R.string.storage_detail_apps,
            R.string.storage_detail_images,
            R.string.storage_detail_videos,
            R.string.storage_detail_audio,
...
// @}
 ...
 /*bird add by liuzhenting 20160815 begin*/
    private static final int[] ITEMS_SHOW_SHARED_COLOR = new int[] {
    ...
    public void update() {
        ...
        /*bird add by liuzhenting 20160815 begin*/
        if (BirdFeatureOption.BIRD_NEW_STORAGE) {
        	/*mItemTotal = buildItem(R.color.memory_system);
            mItemAvailable.setTitle(R.string.memory_total);
            mItemTotal.setSummary(R.string.memory_calculating_size);
            getPreferenceScreen().addPreference(mItemTotal);*/
            Preference mBirdTotalPreference = new Preference(context);
            mBirdTotalPreference.setTitle(R.string.phone_total_title);
            mBirdTotalPreference.setSummary(R.string.phone_total_memory_summary);
            getPreferenceScreen().addPreference(mBirdTotalPreference);
            
            mItemAvailable = buildItem(R.color.memory_avail);
            mItemAvailable.setTitle(R.string.memory_available);
            mItemAvailable.setSummary(R.string.memory_calculating_size);
            getPreferenceScreen().addPreference(mItemAvailable);
        }
        /*bird add by liuzhenting 20160815 end*/
...
// @{ BIRD_SYSTEM_SPACE, add by shenzhiwang, @20160707
        if(BirdFeatureOption.BIRD_SYSTEM_SPACE) {
            float totalSize = BirdUtils.getFloatById(context, R.string.storage_detail_total_size);
            if(totalSize != 0.0f) {
                totalBytes = (long)(totalSize * 1024f * 1024f * 1024f);
            }
        }
// @}
        ...
        /*add by liuzhenting 20160815 begin*/
        if (BirdFeatureOption.BIRD_NEW_STORAGE) {
        	getPreferenceScreen().removePreference(mSummary);
            final BytesResult fresult = Formatter.formatBytes(getResources(), freeBytes, 0);
            mItemAvailable.setSummary(TextUtils.expandTemplate(getText(R.string.storage_size_large),
                    fresult.value, fresult.units));
        }
        /*add by liuzhenting 20160815 end*/
...
// @{ BIRD_SYSTEM_SPACE, add by shenzhiwang, @20160707
        final int[] itemsToAdd = (showShared ? (BirdFeatureOption.BIRD_SYSTEM_SPACE ? BIRD_ITEMS_SHOW_SHARED : ITEMS_SHOW_SHARED) : ITEMS_NO_SHOW_SHARED);
// @}
    ...
    private void addItem(PreferenceGroup group, int titleRes, CharSequence title, int userId) {
        ...
        if (mItemPoolIndex < mItemPreferencePool.size()) {
        	...
        	/*bird add by liuzhenting 20160815 begin*/
            item = buildItem(ITEMS_SHOW_SHARED_COLOR[mItemPoolIndex]);
            /*bird add by liuzhenting 20160815 end*/
    ...
    private StorageItemPreference buildItem(int colorRes) {
    	/*bird add by liuzhenting 20160815 begin*/
    	final StorageItemPreference item;
    	if (BirdFeatureOption.BIRD_NEW_STORAGE) {
    		item = new StorageItemPreference(getActivity(),colorRes);
    	} else {
    		item = new StorageItemPreference(getActivity(),0);
    	}
        /*bird add by liuzhenting 20160815 end*/
        return item;
    }
	...
	//add liuchaofei for change other TotalBytes 20160720 begin
        long totalUsedBytes =0;
        StorageItemPreference other_item = null;
	//add liuchaofei for change other TotalBytes 20160720  end
...
// @{ BIRD_SYSTEM_SPACE, add by shenzhiwang, @20160707
                case R.string.storage_detail_systemspace: {
                    float systemSize = BirdUtils.getFloatById(getActivity(), R.string.storage_detail_system_size);
                    if(systemSize != 0.0f) {
                        long systemBytes = (long)(systemSize * 1024f * 1024f * 1024f);
                        updatePreference(item, systemBytes);
                        totalUsedBytes +=  systemBytes;//add liuchaofei for change other TotalBytes 20160720 end
                    }
                } break;
// @}
                case R.string.storage_detail_apps: {
                    updatePreference(item, details.appsSize.get(userId));
                    totalUsedBytes +=  details.appsSize.get(userId);    //add liuchaofei for change other TotalBytes 20160720 end
                } break;
                case R.string.storage_detail_images: {
                    ...
                    totalUsedBytes +=  imagesSize;     //add liuchaofei for change other TotalBytes 20160720 
                } break;
                case R.string.storage_detail_videos: {
                    final long videosSize = totalValues(details, userId,
                            Environment.DIRECTORY_MOVIES);
                    updatePreference(item, videosSize);
                    totalUsedBytes +=  videosSize;     //add liuchaofei for change other TotalBytes 20160720 
                } break;
                case R.string.storage_detail_audio: {
                    ...
                    totalUsedBytes +=  audioSize;     //add liuchaofei for change other TotalBytes 20160720 
                } break;
                case R.string.storage_detail_other: {
                ...
                case R.string.storage_detail_cached: {
                    ...
                    totalUsedBytes +=  details.cacheSize;     //add liuchaofei for change other TotalBytes 20160720 
                } break;
                case 0: {
                    final long userSize = details.usersSize.get(userId);
                    updatePreference(item, userSize);
                    totalUsedBytes +=  userSize;     //add liuchaofei for change other TotalBytes 20160720 
                } break;
            }
		//add liuchaofei for change other TotalBytes 20160720 begin
        }
        if(other_item != null) {
            updataOtherTotal(totalUsedBytes,other_item);     
		//add liuchaofei for change other TotalBytes 20160720 end	
        }
    }
    ...
    //add liuchaofei for change other TotalBytes 20160720 begin
    private  void updataOtherTotal(long totalUsedBytes, StorageItemPreference other_item ){
        final Context context = getActivity();
        final File file = mVolume.getPath();
        ...
        final long freeBytes = file.getFreeSpace();
        final long usedBytes = totalBytes - freeBytes;
        long otherBytes = usedBytes - totalUsedBytes;
        updatePreference(other_item, otherBytes);
    }
    //add liuchaofei for change other TotalBytes 20160720 end
