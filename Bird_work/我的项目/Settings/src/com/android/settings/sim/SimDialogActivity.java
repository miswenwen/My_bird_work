...
/*BIRD_A200_CUSTOM custom A200 sim indicatore by lichengfeng @20160406 begin */
...
/*BIRD_A200_CUSTOM custom A200 sim indicatore by lichengfeng @20160406 end */
public class SimDialogActivity extends Activity {
    private static String TAG = "SimDialogActivity";

	/*BIRD_A200_CUSTOM custom A200 sim indicatore by lichengfeng @20160406 begin */	
	private static final String HIDE_SIM_INDICATOR_ENABLE = "hide_sim_indicator_enable";
	/*BIRD_A200_CUSTOM custom A200 sim indicatore by lichengfeng @20160406 end */
	...
	private int getUserSelectedOutgoingPhoneAccountId() {
		final TelecomManager telecomManager = TelecomManager.from(this);
		PhoneAccountHandle phoneAccount = telecomManager.getUserSelectedOutgoingPhoneAccount();
		if (phoneAccount != null) {
			return TelephonyUtils.phoneAccountHandleTosubscriptionId(this, phoneAccount);
		} else {
			return -1;
		}
	}
	...
	private void setStatusBarIndiactor(boolean isShowStatusBarIndicator , int subId) {
	    try {
            IStatusBarService statusbar = getStatusBarService();
            if (statusbar != null && isShowStatusBarIndicator) {
				// HIDE_SIM_INDICATOR_ENABLE = 0 can hideSimIndicator
				// HIDE_SIM_INDICATOR_ENABLE = 1 can not hideSimIndicator
				Settings.System.putInt(getContentResolver(), HIDE_SIM_INDICATOR_ENABLE , 1);
				statusbar.showDefaultAccountStatus(subId);
			} else {
				Settings.System.putInt(getContentResolver(), HIDE_SIM_INDICATOR_ENABLE , 0);
    ...
    private void setDefaultSmsSubId(final Context context, final int subId) {
		android.util.Log.i("lcf_simdialog","setDefaultSmsSubId subId: "+subId);
        final SubscriptionManager subscriptionManager = SubscriptionManager.from(context);
        /// M: [C2K Solution 1.5] @{
		...
		if (BirdFeatureOption.BIRD_A200_CUSTOM) {
			int phoneAccountId = getUserSelectedOutgoingPhoneAccountId();
			android.util.Log.i("lcf_simdialog","setUserSelectedOutgoingPhoneAccount phoneAccountId: "+subId);
			if (phoneAccountId == subId && subId != -1) { 
				setStatusBarIndiactor(true,phoneAccountId);
			} else {
				setStatusBarIndiactor(false,-1);
			}
		}
		/*BIRD_A200_CUSTOM custom A200 sim indicatore by lichengfeng @20160406 end */
        }
    ...
    private void setUserSelectedOutgoingPhoneAccount(PhoneAccountHandle phoneAccount) {
		...
		if (BirdFeatureOption.BIRD_A200_CUSTOM) {
			...
			android.util.Log.i("lcf_simdialog","setUserSelectedOutgoingPhoneAccount subId: "+subId);
			if (phoneAccount != null && (TelephonyUtils.phoneAccountHandleTosubscriptionId(this, phoneAccount) == subId) && (subId != -1)) {
				setStatusBarIndiactor(true,subId);
			} else {
				setStatusBarIndiactor(false,-1);
			}
		}
